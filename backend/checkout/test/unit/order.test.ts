...
test("Deve criar um pedido com 3 items", function() {
    const order = new Order("772.801.132-49");
    order.addItem(new Product(1, 'A', 10, 100, 30, 10, 3), 1);
    order.addItem(new Product(2, 'B', 10, 50, 50, 50, 22), 1);
    order.addItem(new Product(3, 'C', 10, 10, 10, 10, 0.9), 2);
...
test("Deve validar se possui algum item repetido", function() {
    const order = new Order("772.801.132-49");
    order.addItem(new Product(1, 'A', 10, 100, 30, 10, 3), 1);
    expect(() =>  order.addItem(new Product(1, 'B', 10, 50, 50, 50, 22), 1)).toThrow(new Error("Has repeated item"));
});


test("Deve validar se possui algum item com quantidade negativa", function() {
    const order = new Order("772.801.132-49");
    expect(() =>  order.addItem(new Product(1, 'B', 10, 50, 50, 50, 22), -1)).toThrow(new Error("Invalid product quantity"));
});

test("Deve validar se possui algum produto com dimensões negativas", function() {
    const order = new Order("772.801.132-49");
    expect(() =>  order.addItem(new Product(1, 'B', 10, -1, 50, 50, 22), 1)).toThrow(new Error("Item with negative dimension"));
});
